...
DEBT = [
    {
        ...
        "order": -3,
        "title": "Card 5 (rbac-gxp) shipped the audit screen + save_settings diff + Henry's chat audit_query, NOT device-token TTL or henry_broker's autonomous-escalation audit context",
        "status": "open",
        "what": "Built: events.save_settings() now emits an old->new diff "
                "event per write (changed top-level keys only, secrets masked "
                "by key-name regex - spine/storage/events.py). "
                "events.query_audit() is the one shared filter GET /audit "
                "(routes_audit.py) and Henry's new 'audit_query' chat action "
                "(cells/copilot/copilot_actions.py) both call. The app got a "
                "real screen (surfaces/app/src/app/audit.tsx, cap-gated "
                "audit.read, filters: kind/actor/q). NOT built: (1) device "
                "token TTL + surfacing '>90 days unused' - auth.py's tokens "
                "still have no expiry field and no staleness marker anywhere "
                "in the UI. (2) henry_broker.py's autonomous escalation path "
                "(_snapshot()/_decide()) does NOT automatically pull related "
                "audit events into its context - only the INTERACTIVE chat "
                "surface got the audit_query action; an autonomously-"
                "escalating card cannot yet self-report e.g. 'blocked since "
                "GxP activation 2h ago by duy' the way the plan's 'Consumer "
                "2' envisioned.",
        "why_it_bites": "An auditor reviewing device access has no signal for "
                        "a token issued months ago and never revoked - "
                        "'access review' (a real GxP/Part-11 requirement, see "
                        "ops/docs/backlog/rbac-gxp card 5's own acceptance "
                        "criteria) is not actually satisfiable from the UI "
                        "yet. Separately, Henry's autonomous judgement on a "
                        "GxP/SoD-blocked escalation is less informative than "
                        "it could be - it sees the card snapshot but not the "
                        "audit history that would explain WHY a block exists.",
        "trigger": "An owner asking 'which device tokens are stale' or "
                   "expecting Henry to explain a block's history unprompted "
                   "during an autonomous escalation, rather than only when "
                   "asked directly in chat.",
        "fix": "(1) Add an optional expiry to auth.issue_token, and a "
               "computed 'stale' flag (last_seen > 90d) surfaced in the /users "
               "response + a UI marker (probably the existing Users panel, "
               "not the audit screen). (2) Extend henry_broker.py's "
               "_snapshot() to call events.query_audit(track=tid) for the "
               "escalating card and fold a short summary into the judgement "
               "prompt - same gate question as the chat action (is the "
               "relevant actor's role audit.read-capable), but keyed off the "
               "card's dispatcher/owner since Henry itself has no session role.",
    },
    {
        ...
        "what": "ops/docs/backlog/rbac-gxp card 2 built spine/auth/permissions.py "
                ...
                "and server.py's own /users block + PATTERNS entries for /users, "
                "/users/<name>/<action>. NOT yet migrated: the remaining ~15-20 "
                "route modules with inline role checks - cells/engineer/"
                "routes_tracks.py, routes_track_actions.py, cells/pm/routes_pm.py, "
                "cells/copilot/routes_copilot.py + copilot_actions.py's separate "
                "chat-verb role gate (settings.policy.chat_admin_roles, a "
                "DIFFERENT store than the new matrix - card 2's design explicitly "
                "deferred unifying these), cells/connectors/routes_connectors.py, "
                "routes_checkpoints.py, routes_cells.py, routes_projects.py, "
                "routes_system.py, routes_info.py, and server.py's own biggest "
                "block: the client-blanket-POST allowlist (do_POST, 'clients can "
                "file and comment only') and the large /tracks/* path-param "
                "if-chain (~25-30 checks, card 2's own plan flagged this as the "
                "highest-blast-radius block, saved for last on purpose).",
