...
DEBT = [
    ...
    },
    {
        ...
        "order": -9,
        "title": "only the BOARD Henry session is proactively compacted/pruned - card sessions are not",
        "status": "open",
        "what": "Henry conversations are keyed per (user, card) since 2026-09-02 "
                ...
                "does. _schedule_prune (2026-09-15, the between-turns "
                "tool-result pruning) inherits the SAME board-only scope, "
                "same call shape (sid = _sessions().get(_skey(user))) - a "
                "deliberate copy, not an oversight, so both maintenance jobs "
                "share this one entry instead of two near-duplicate ones.",
        ...
        "trigger": "A single card with a very long Henry conversation - most "
                   ...
                   "returning to.",
        "fix": "Give _maybe_compact (and _schedule_prune/_do_prune) the same "
               "per-conversation key the rest of the path now uses: act on "
               "whichever session the turn belongs to, not only the board "
               "one. The mark/window logic is already per-session (it reads "
               "ctx_tokens from copilot_stats), so this is a keying change, "
               "not new machinery - but copilot_stats is itself still keyed "
               "per USER, so that has to be split first or a card turn will "
               "compact/prune against the board's numbers.",
        "since": "2026-09-02",
    },
    {
    ...
    },
    {
        ...
        "what": "FURTHER CLOSED by henry-memory-db-authority + its same-day follow-up: "
                ...
                "table), written via a <memory-save>/<memory-delete> sentinel in his "
                "own turn output (cells/copilot/chat/copilot_memory.py), digested into "
                "every board turn, and read in full via ops/tools/henry_memory_get.py "
                "- Henry's Read/Write tools are no longer part of the memory path at "
                "all, in either direction. The claude CLI ALSO carries its own "
                "auto-memory directory under "
    ...
    },
    {
        ...
        "what": "Measured 2026-09-21 while answering 'wie nehme ich alles mit': "
                ...
                "115 notes, 7 users, 406 cards, chat, provenance columns), but "
                "the CLI auto-memory has no path off this machine. 92 notes in "
                "~/.claude/projects/<slug>/memory/, in no export, and the slug "
                "is derived from the MAIN repo path so even a hand copy lands "
                "in the wrong folder on a machine where the repo sits "
                "elsewhere. Since 2026-09-21 those notes rank owner-fact - the "
                "highest rank we have.\n"
                "CORRECTION, same day: the first version of this entry said "
                "'there is no .git anywhere under ~/.claude (checked)'. That "
                "was FALSE and the check was at the wrong level - `git -C "
                "~/.claude status` is indeed not a repository, but the MEMORY "
                "DIRECTORY ITSELF is: 123 commits back to 2026-08-16, messages "
                "of the form 'memory: N file(s) changed', clean tree, and NO "
                "REMOTE. The CLI versions its own memory. So the loss risk is "
                "not 'no history' - it is 'no second copy'. Found by reading "
                "what takeout.py actually copied (599 files where 92 notes "
                "were expected), not by re-reasoning.",
        ...
        "trigger": "Any second machine, any restore, or the takeout feature "
                   "being built.",
        "fix": "CHEAPEST FIRST, now that it is known to be a real repo: give "
               "that repo a private remote and push it. That is a backup with "
               "history for the cost of one `git remote add`, and it needs no "
               "archive at all. ops/tools/takeout.py already carries the "
               "directory INCLUDING its .git, so the history survives a move "
               "either way. On restore, write it to the path the CLI ITSELF "
               "reports (copilot_memory.observe_auto_dir folds memory_paths.auto "
               "from the init frame) - never to a constructed slug. That write "
    ...
    },
    {
        ...
        "what": "PAID - kept for why the write path looks the way it does. "
                ...
                "'memory' save/delete event per mutation. The digest itself is now "
                "injected ONLY at session establishment (a fresh spawn, a rotated/"
                "detached resume, or an in-place compaction - tracked via a "
                "(sid, compacted_at_turn) marker since a real /compact keeps the same "
                "sid) instead of every turn - it used to ride every board turn "
                "regardless of whether the conversation was a fresh session or a warm "
                "resume whose transcript already contained it. _fold_memory_to_db is "
                "deleted entirely (net-negative diff, not kept as a fallback). SAME-DAY "
                "FOLLOW-UP: the first cut still kept daemon/henry_memory/ as a "
                "disposable READ cache (regenerate_cache() clobbering it from the db at "
                "session establishment, so Henry's Read tool could open a full note). "
                "Owner review: a directory that looks identical before and after "
                "'db-authoritative' ships is a skeleton in the closet, not a visible "
                "fix. regenerate_cache() and MEMORY_DIR are deleted too; "
                "ops/tools/henry_memory_get.py (list|get) is the only way a full note "
                "is ever read - a read-only db query through Henry's existing "
                "Bash-allowlist pattern (same shape as board_state.py/loop_state.py). "
                "Memory now has NO filesystem surface at all, not even a disposable one.",
