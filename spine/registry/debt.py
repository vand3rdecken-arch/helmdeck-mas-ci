...
DEBT = [
    {
        ...
        "what": "ops/docs/backlog/remote-device-execution: a team member's own "
                ...
                "capture exists. STILL OPEN, narrowed to exactly Phase D: no "
                "packaging/install story, shells out to `claude` directly "
                "instead of reusing spine/agent/drivers.py's turn machinery "
                "(so a device turn still reports no usage/cost numbers - "
                "billing_scope is where that capture will plug in), device "
                "revocation does not interrupt an in-flight task, and the "
                "board UI does not yet surface a stuck/reassignable device "
                "card (the API + daemon-side event exist, no screen renders "
                "them).",
        "why_it_bites": "Without Phase D, a device turn's real Claude usage/"
                        "cost is invisible to the app even though "
                        "billing_scope now correctly keeps it OUT of the "
                        "shared account's numbers - the owner can see THAT a "
                        "device did work, not what it cost. A revoked "
                        "device's OS process also keeps running (just loses "
                        "its next API call) rather than being told to stop.",
        "trigger": "wanting real per-device cost visibility, or an install/"
                   "autostart story beyond running the script by hand",
        "fix": "Extract the pure spawn-argv + stream-parse core of a turn "
               "from spine/agent/drivers.py into something hd_worker.py can "
               "call without daemon-side trackstore/run_dir state, so a "
               "device turn reports usage/cost through billing_scope and can "
               "stream its transcript back for the board to show live; add a "
               "packaged install/autostart story; make revoke() actively "
               "signal the worker to stop rather than only invalidating its "
               "next call; surface stuck/reassignable device cards in the "
               "board/History UI.",
        "order": 0,
    },
    {
