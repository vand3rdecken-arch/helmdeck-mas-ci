...
DEBT = [
    {
        ...
        "what": "ops/docs/backlog/remote-device-execution: a team member's own "
                ...
                "to 61. STILL OPEN: no pip entry point / Windows Scheduled "
                "Task or service registration (config file exists, nothing "
                "installs it for you), no live transcript streaming mid-"
                "turn (needs the relay long-poll shape, a bigger lift, "
                "unchanged from the original plan), device revocation does "
                "not interrupt an ALREADY-RUNNING turn (only stops the next "
                "poll), and the board UI does not yet surface a stuck/"
                "reassignable device card (the API + daemon-side event "
                "exist, no surfaces/app screen renders them - a frontend "
                "task, out of scope for this backend-only round).",
        "why_it_bites": "The install/autostart gap means running a device "
                        "worker is still a manual 'keep a terminal open' "
                        "affair, not a background service. Revocation "
                        "stops a worker's NEXT poll but not a turn already "
                        "in flight. No board-UI visibility means a stuck "
                        "device card is only discoverable via the API/logs, "
                        "not the app the owner actually looks at day to "
                        "day.",
        ...
        "fix": "Add a packaged install/autostart story (pip entry point or "
               ...
               "--config); make revoke() interrupt an in-flight turn, not "
               "just the next poll; stream a device turn's live transcript "
               "back over the queue channel (needs the relay long-poll "
               "shape); surface stuck/reassignable device cards in the "
               "board/History UI.",
        "order": 0,
    },
    {
