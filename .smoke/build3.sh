...
printf 'sdk.dir=%s\n' 'C:/Users/Tien Duy Vo/AppData/Local/Android/Sdk' > app/android/local.properties
node app/plugins/withLanCleartext.js app/android || { echo "withLanCleartext FAILED"; exit 1; }

echo "== drop stale .cxx caches (they reference the long C:\\ path)"
find app/node_modules -maxdepth 4 -type d -name .cxx -prune -exec rm -rf {} + 2>/dev/null
rm -rf app/android/app/.cxx app/android/.cxx
