...
https://gis-bob-aspect-communities.trycloudflare.com   ← cloudflared quick tunnel
   │  http 127.0.0.1:6795
   ▼
relay/relay.py  ── /updates/manifest + /updates/assets aus .smoke/updates
...
- **Fund 1** — ein fehlgeschlagener Resume-Check verbrennt das volle
  10-Minuten-Fenster (`app/src/data/ota.ts:42-43`). Kosmetisch, aber es
...
Marker, den `deploy/rollback_update.sh --embedded` auf dem echten Relay
...
`relay/relay.py`, `_build_manifest()`:
...
Fix-Vorschlag (klein): `deploy/push_update.sh` leitet die RTV heute schon ab
...
- **Der Emulator ist geteilte Infrastruktur.** Mitten im Lauf hat eine fremde
  Pipeline (`deploy/build_apk.sh` macht `adb uninstall` + `adb install` als
