...
# incident debt (daemon/spine/debt.py). Each copy silently assumed the
# FILE'S OWN location (flat in daemon/), so moving any file into a subfolder
# (daemon/cells/<id>/, daemon/spine/) would have broken its path math with
...
#
# daemon/ is a REAL Python package now (import daemon.cells.pm.pm, not a
