...
DEBT = [
    ...
    },
    {
        ...
        "what": "Owner pushback on the just-shipped Cell-folder reorg "
                "(daemon/cells/<id>/ + daemon/spine/): daemon/_subpaths.py's "
                ...
                "daemon/__init__.py + daemon/spine/__init__.py + "
                "daemon/spine/routes/__init__.py + daemon/cells/<id>/"
                ...
                "absolute `from daemon.spine import X` / "
                "`from daemon.cells.<id> import X` (mechanical, via a "
                ...
                "`distill`, so `from daemon.spine.distill import distill` "
                ...
                "folder is also named pm, so `daemon.cells.pm` legitimately "
                ...
                "daemon/spine/cells.py's `start=`/`route_modules=` tuples "
                ...
                "of latent gap - only searched daemon/cells/<id>/ and flat "
                "daemon/, never daemon/spine/ or daemon/spine/routes/, so "
                ...
                "import became `from daemon.spine import events` (a "
                ...
                "stub now targets `sys.modules[\"daemon.spine.events\"]` "
        ...
        "fix": "PAID. Verified: full 17-file daemon test suite green "
               ...
               "daemon.spine.cells manifest/_cell_routes()/read_source() "
               "all confirmed working for every cell including cross-cell-"
               "owned route modules (process's routes_misc.py); "
               "daemon.cells.engineer.sessions (the crown jewel, heaviest "
               ...
               "daemon.spine.auth.policy and its internal events import "
               "target moved to daemon.spine.storage.events, so "
