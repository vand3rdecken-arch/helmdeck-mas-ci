...
def _execute(action, card, lane, text, esc):
    ...
    if action == "ignore":
        return True
    # GxP: Henry's two HANDS verbs are off in the mode. `move` would also be
    # stopped by the lane machine's chokepoint (he is not an account), but it is
    # refused here too so the escalation stays open and visibly waiting for a
    # person, instead of being closed against a landing that never happened.
    # `did` has no chokepoint at all - it is Henry editing the live tree - so
    # this is the only place it can be stopped.
    if action in ("did", "move") and gxp.disabled("henry_" + action):
        from daemon.spine.storage import events
