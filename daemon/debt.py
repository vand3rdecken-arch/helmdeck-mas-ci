...
DEBT = [
    ...
    },
    {
        ...
        "title": "Daemon god-file breakup — 21 service modules extracted, "
                 "server.py route dispatch down to a small residual grab-bag "
                 "(the tracks/dispatch crown jewel is DONE) + pm.py stateful "
                 "notices remain",
        "status": "open",
        ...
        "why_it_bites": "server.py is down to 740 lines with 14 groups now "
                        "extracted (routes_auth/policy/settings/glance/info/pm/"
                        "misc/control/relay/connectors/checkpoints/projects/"
                        "copilot/tracks/track_actions.py - the 2026-08-18 "
                        "session's LAST entry converted the tracks/dispatch "
                        "crown jewel: routes_tracks.py [14 CRUD/read routes] + "
                        "routes_track_actions.py [5 gate/dispatch-critical "
                        "routes: /tracks/<id>/stream,steer,answer,cancel,lane - "
                        "kept in their own module for extra scrutiny since they "
                        "sit directly on lanemachine.py's move_lane -> _gate -> "
                        "_merge_to_main]. Verified with a real end-to-end "
                        "dispatch/gate/bounce round trip (test_server_routes.py, "
                        "not just role-gate assertions) - see the 14th-slice "
                        "paragraph above for what was tested and the 3 real bugs "
                        "(cardadmin.py/dispatch.py missing imports, runs.REC "
                        "sandboxing) it found and fixed along the way. The "
                        "dispatch-table pattern is now proven SAFE 14 TIMES "
                        "RUNNING (full route-smoke coverage, full 16-file suite "
                        "green, real-file integrity verified - no tracked "
                        "state file touched). What remains in server.py is a "
                        "genuinely small residual grab-bag, not another crown "
                        "jewel: presence/push/harness/debt-fix/import routes, "
                        "the runs/timeline/video/videochunk GET group, and the "
                        "/processes/<id>/step path-param sub-router - each "
                        "would still need its own routes_*.py + smoke-test "
                        "coverage before conversion, so a bug in one of them "
                        "stays invisible to test_server_routes.py until it is "
                        "added, but none of them touch gate/merge state.",
        "trigger": "adding a new HTTP route (touches the still-large if-chain); "
                   "any claim the daemon's API surface is fully modular",
        "fix": "server.py: the crown jewel (tracks/dispatch) is DONE - what's "
               "left is the small residual grab-bag (presence/push/harness/"
               "debt-fix/import routes, runs/timeline/video/videochunk GET, "
               "/processes/<id>/step). Repeat the proven routes_auth.py/.../"
               "routes_tracks.py pattern for each: (1) add smoke-test coverage "
               "for that group's routes to test_server_routes.py FIRST, (2) "
               "move the bodies verbatim into routes_<concern>.py as (self, "
               "user[, body]) functions (watch for module-level helpers/consts "
               "that need to move WITH the group), (3) add the dispatch-table "
               "check (GET_PREFIX_ROUTES for any .startswith() route, or an "
               "inline path-param guard per the routes_checkpoints.py/"
               "routes_tracks.py precedent for parts[]-shaped routes), (4) "
               "verify full suite + events.jsonl integrity + a live-daemon curl "
               "smoke pass. None of these remaining groups touch gate/merge "
               "state, so they carry materially lower risk than the slice that "
               "just shipped - server.py's breakup can reasonably be called "
               "DONE for the load-bearing/high-risk surface, with only routine "
               "cleanup left. "
