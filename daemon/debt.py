...
DEBT = [
    ...
    },
    {
        ...
        "why_it_bites": "server.py is down to ~1165 lines with 9 groups now "
                        "extracted (routes_auth/policy/settings/glance/info/pm/"
                        "misc/control/relay.py - control.py 2026-08-18 added GET "
                        "/control/state + POST /control/teach/start,stop, "
                        "/control/distill, /control/demo; relay.py added POST "
                        "/relay/pair,unpair) - the dispatch-table pattern is "
                        "proven SAFE 9 TIMES RUNNING (full route-smoke coverage, "
                        "full 16-file suite green, md5sum-verified real-file "
                        "integrity after every group) but the STILL-INLINE "
                        "remainder is real, repetitive work: tracks/dispatch [the "
                        "crown jewel - touches sessions.new_track/move_lane/"
                        "lanemachine; new_track's ASCII-slug branch logic and the "
                        "answer/steer/cancel/lane sub-router are all still in "
                        "server.py], chat/copilot (/chat, /chat/cancel, /chat/"
                        "history, /chat/live), connectors (list/rollback/run), "
                        "checkpoints (list/diff/restore), processes (the /"
                        "processes/<id>/step path-param sub-router - GET /"
                        "processes and POST /processes/new already moved via "
                        "routes_misc.py), projects CRUD, presence/push/harness/"
                        "debt-fix/import routes, the runs/timeline/video/"
                        "videochunk GET group, and the tracks/<id>/{live,turns,"
                        "history,transcript,checkpoints,attachments,attachment} "
                        "GET sub-router - each needs its own routes_*.py + "
