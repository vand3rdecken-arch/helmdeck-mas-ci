...
DEBT = [
    ...
    },
    {
        ...
        "what": "The full-dynamism decree made the APP a plugin composition; this "
                ...
                "\n"
                "server.py 2109 -> 1719, 3 modules (module-level helpers only): "
                "glances.py (/glance payload), apimeta.py (lane/loop map + config "
                "schema), startup.py (tls/env-hydrate/singleton-lock).\n"
                "\n"
        ...
        "why_it_bites": "What's genuinely left is not more of the same mechanical "
                        "extraction - it is TWO structural changes with materially "
                        "different risk profiles. (1) server.py's H handler class: "
                        "66+ route branches inline as `if p == \"/x\":` using "
                        "self._send/self._user/self._sid - converting this to a "
                        "route-table needs either a RequestContext object threaded "
                        "through free functions, or per-concern mixin classes. "
                        "This is the daemon's live HTTP surface with the THINNEST "
                        "test coverage (only test_transport_tls.py touches "
                        "server.py directly) - a silent route-dispatch bug would "
                        "surface only when the UI hits that specific endpoint, "
                        "exactly the failure mode the modularity work is trying to "
                        "prevent, not risk. (2) pm.py's remaining notice/watch "
                        "functions (_usage_checkin, _plan_gate_notice, "
                        "_triangle_watch, _cost_watch, review_burn cluster) share "
                        "mutable state (_last_touch, loopstate) and pm-internal "
                        "helpers (_ask/_say/_activity) - each needs multiple lazy "
                        "back-references for modest-sized clusters, lower ROI than "
                        "the sessions.py/drivers.py work.",
        "trigger": "adding a new HTTP route (touches the God-class); any claim "
                   "the daemon's API surface is modular; the next pm.py notice "
                   "function that needs editing",
        "fix": "server.py: EITHER (a) add route-level smoke tests first (spin the "
               "daemon, hit each endpoint, assert shape) so a mixin-class or "
               "route-table refactor is verifiable, OR (b) do the safer mixin-"
               "class split (group routes by concern into modules, each a class "
               "H inherits from - preserves self._send/_user exactly, no route "
               "body changes) and verify via careful manual review + the existing "
               "test_transport_tls.py + a live-daemon smoke pass before shipping. "
               "pm.py: thread a small State object (loopstate + _ask/_say/"
               "_activity) into the notice functions instead of module globals, "
               "OR accept the multi-back-ref cost per cluster (review_burn/"
               "_burn_judge/_push_burn/_review_burn is the next candidate, ~90 "
               "lines, 5 back-refs).",
        "order": 31,
    },
]
