...
DEBT = [
    ...
    },
    {
        ...
        "fix": "(1) DONE: processes.py migrated into db.py (removes its ROOT/"
               ...
               "(3) OPEN: connectors.py (CDIR), checkpoints.py (CPDIR + a "
               "settings.json/users.json path dict), and voice.py (CACHE) are "
               "the confirmed remaining candidates (grep-verified) - migrate "
               "each into db.py (preferred - "
               "removes the bug class permanently, same pattern as this commit) "
               "or add it to a SHARED sandbox helper (a single "
               "`sandbox_daemon_storage(tmp)` function every test calls, so a "
               "newly-added module's path only needs patching in ONE place, not "
               "re-derived per test file). (4) OPEN: verify test integrity checks "
               "with md5sum/checksum, not file-size+mtime - the latter cannot "
               "distinguish 'untouched' from 'grew by a plausible amount'.",
        "order": 32,
    },
]
