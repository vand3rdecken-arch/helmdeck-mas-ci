...
DEBT = [
    ...
    },
    {
        ...
        "what": "The full-dynamism decree made the APP a plugin composition; this "
                ...
                "\n"
                "server.py 2109 -> ~1645, 5 modules: glances.py (/glance payload), "
                "apimeta.py (lane/loop map + config schema), startup.py (tls/env-"
                "hydrate/singleton-lock) - module-level helpers; PLUS the H "
                "handler's do_GET/do_POST if-chain now has a real DISPATCH-TABLE "
                "seam: routes_auth.py (5 routes: auth/state/setup/register/login/"
                "logout) and routes_policy.py (3 routes: /policy, /policy/swap, "
                "/reconfig/track) are the first two route groups converted.\n"
                "\n"
        ...
        "why_it_bites": "server.py still has ~58 of its ~66 route branches inline "
                        "in the do_GET/do_POST if-chain - the dispatch-table "
                        "pattern is proven safe (2 groups converted, full-suite + "
                        "route-smoke verified) but converting the rest is real, "
                        "repetitive work: each remaining group (tracks/dispatch, "
                        "machine, glance/glasses, settings/config, chat/copilot, "
                        "processes, attachments/checkpoints) needs its own "
        ...
        "fix": "server.py: repeat the routes_auth.py/routes_policy.py pattern for "
               "each remaining concern group, one PR each: (1) add smoke-test "
               "coverage for that group's routes to test_server_routes.py FIRST, "
               "(2) move the bodies verbatim into routes_<concern>.py as (self, "
               "user[, body]) functions, (3) add the dispatch-table check, "
               "(4) verify full suite + events.jsonl integrity + a live-daemon "
               "curl smoke pass. Candidates in rough size/risk order: settings/"
               "config (small, low-risk), glance/glasses (medium, already has "
               "glances.py module-level helpers to lean on), tracks/dispatch "
               "(large, touches sessions.new_track/move_lane - highest value). "
