...
DEBT = [
    ...
    },
    {
        ...
        "why_it_bites": "server.py is down to 1073 lines with 13 groups now "
                        "extracted (routes_auth/policy/settings/glance/info/pm/"
                        "misc/control/relay/connectors/checkpoints/projects/"
                        "copilot.py - the 2026-08-18 session added the last four: "
                        "connectors.py [GET /connectors, POST /connectors/<name>/"
                        "rollback,run], checkpoints.py [GET /checkpoints, GET "
                        "/checkpoints/<id>/diff, POST /checkpoints/<id>/restore, "
                        "verified with a real create/list/diff/restore round-"
                        "trip against a sandboxed CPDIR - not just role-gate "
                        "assertions], projects.py [/projects CRUD], copilot.py "
                        "[/chat, /chat/cancel, /chat/history, /chat/live - the "
                        "real model turn is never invoked by the test]) - the "
                        "dispatch-table pattern is proven SAFE 13 TIMES RUNNING "
                        "(full route-smoke coverage, full 16-file suite green, "
                        "md5sum-verified real-file integrity after every group) "
                        "but the STILL-INLINE remainder is real, repetitive, and "
                        "now concentrated almost entirely in ONE cluster: tracks/"
                        "dispatch [the crown jewel, DELIBERATELY DEFERRED - "
                        "touches sessions.new_track/move_lane/lanemachine; "
                        "new_track's ASCII-slug branch logic, the answer/steer/"
                        "cancel/lane sub-router, track CRUD (archive/fork/fork-"
                        "chat/delete/update/rewind/attach), history/live/turns/"
                        "transcript/checkpoints/attachments GET sub-routers, and "
                        "the /processes/<id>/step path-param sub-router are all "
                        "still in server.py - re-read sessions.py's public "
                        "surface (lanemachine.py/dispatch.py/cardadmin.py) "
                        "before touching it, per the task brief that shipped "
                        "this slice], plus a residual grab-bag (presence/push/"
                        "harness/debt-fix/import routes, the runs/timeline/"
                        "video/videochunk GET group) - each needs its own "
                        "routes_*.py + smoke-test coverage before conversion, so "
                        "a bug in an unconverted route stays invisible to "
                        "test_server_routes.py until that group is added.",
