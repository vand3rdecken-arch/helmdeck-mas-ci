...
DEBT = [
    ...
    },
    {
        ...
        "what": "The full-dynamism decree made the APP a plugin composition, but "
                ...
                "SEAMS DONE (14, all FULL-suite 11/11 verified): from sessions.py "
                "-> gitutil.py (git primitives), econ.py (turn economics), "
                "blockers.py ('what needs the owner'), outcomes.py (outcome "
                "extraction), worktrees.py (reclaim/sweep) => sessions.py "
                "3927->3469; from drivers.py -> agentcli.py (argv/MCP) => "
                "1490->1317; from copilot.py -> copilot_stats.py (PM econ) => "
                "1136->1051 + copilot_actions.py (reply/action parsing) => 1012; "
                "from drivers.py -> proctable.py (PID/process table, "
                "MONKEYPATCHED - no test change needed, patched fns' callers stay "
                "in drivers) + spawnenv.py (_env/_card_env) => drivers.py "
                "1490->990 (NO LONGER a god-file); from pm.py -> pm_budget.py "
                "(budget/quota math) => 2192->2079; from server.py -> glances.py "
                "(/glance payload), apimeta.py (lane/loop map + config schema), "
                "startup.py (tls/env-hydrate/singleton) => 2109->1719 (all "
                "MODULE-LEVEL helpers; the H handler class + its do_GET/do_POST "
                "route dispatch is untouched - that split is structural). "
                "BOUNDARY FOUND (attempted + reverted): the machine-task cluster "
                "(machine_policy/new_machine_task/new_direct_task/_start_machine) "
                "was tried via the circular-import pattern (module `from sessions "
                "import <N>` at top, sessions imports it at bottom) - but it needs "
                "19 back-references to sessions (_mutate/_turn/move_lane/steer/...) "
                "and the line range sweeps an unrelated outcome const. That is a "
                "FRAGILE pseudo-module (borrows ~everything from its parent), not "
                "real modularity - reverted. Conclusion: sessions.py's remaining "
                "core (turn lifecycle, gate/merge, move_lane state machine, "
                "machine dispatch, steer, bg sweep) is ONE cohesive orchestrator; "
                "further splitting needs a DESIGN refactor (a TrackContext/store "
                "object threaded through, or a route framework for server.py's H), "
                "NOT mechanical re-export extraction. "
                "CORRECTION (owner reframe): that 'boundary' was WRONG - the "
                "coupling was TRAPPED logic, not a natural seam. The machine-task "
                "back-references only existed because the shared CORE it depends "
                "on was still inside the sessions monolith. Fix = extract the "
                "SERVICES bottom-up (moving each service's test patch point with "
                "it), so higher modules become thin dependents (DeepSeek "
                "kernel/plugin model). PROVEN: trackstore.py (data layer _load/"
                "_save/_mutate; the 5 suites now patch trackstore._db) + locks.py "
                "(turn/desktop/direct locks + steer epoch) extracted as SERVICES, "
                "FULL suite green. NEXT services (same technique, monkeypatched -> "
                "move patch point): turn-runner (_turn/_finish_turn/_settle_reply), "
                "lane/gate/merge machine (move_lane/_gate/_merge_to_main/_repo_hook "
                "- patched sessions._gate/_merge_to_main/_autocommit/_repo_hook -> "
                "the module). Then re-do machine-task: its back-refs collapse to a "
                "few named service deps. Goal: whole module graph enumerable + "
                "streamable to the UI for the user to configure. "
                "All via lazy back-imports "
                "where a moved fn needs a home-module helper (no cycle). "
                "REMAINING: server.py (2109, split "
                "do_GET/do_POST dispatch into route modules - structural), pm.py "
                "(2192, state-threading), and the MONKEYPATCHED clusters "
                "(gate/merge _gate/_merge_to_main, drivers PID _tree_kill/_record_"
                "pid, sessions data layer _load/_mutate) which need their test "
                "patch target moved with the cluster.",
