...
DEBT = [
    ...
    {
        "id": "daemon-god-files",
        "title": "Daemon god-file breakup — 21 service modules extracted, "
                 "server.py route dispatch + pm.py stateful notices remain",
        "status": "open",
        "what": "The full-dynamism decree made the APP a plugin composition; this "
                ...
                "\n"
                "server.py 2109 -> ~1304 (38% cut), 8 modules total: glances.py "
                "(/glance payload), apimeta.py (lane/loop map + config schema), "
                "startup.py (tls/env-hydrate/singleton-lock) - module-level "
                "helpers; PLUS the H handler's do_GET/do_POST if-chain now has a "
                "real DISPATCH-TABLE seam with FIVE route groups converted (23 "
                "routes total): routes_auth.py (5: auth/state/setup/register/"
                "login/logout), routes_policy.py (3: /policy, /policy/swap, "
                "/reconfig/track), routes_settings.py (5: /settings GET+POST, "
                "/nightshift, /usage, /automation), routes_glance.py (4: /glance, "
                "/glance/voice/<id>.mp3 [first PREFIX route - GET_PREFIX_ROUTES, "
                "an ordered list checked before the exact-match dicts], /glance/"
                "talk, /glance/answer), routes_info.py (6 read-only: /debt, "
                "/charter, /loop/map, /models, /harness, /harness/schema - also "
                "exposed + removed a pre-existing DEAD duplicate /harness block "
                "further down do_GET, unreachable since the dispatch check runs "
                "first). 23 routes converted; ~43 of the original ~66 remain "
                "inline.\n"
                "\n"
                ...
                "\n"
                "PATTERN (repeatable, proven twice): each route body moves "
                "VERBATIM into a routes_<concern>.py free function taking "
                ...
                "identical. Verify with test_server_routes.py + the full suite + "
                "an events.jsonl integrity check after EVERY group.\n"
                "\n"
        ...
        "why_it_bites": "server.py still has ~43 of its ~66 route branches inline "
                        "in the do_GET/do_POST if-chain - the dispatch-table "
                        "pattern is proven SAFE 5 TIMES RUNNING (auth/policy/"
                        "settings/glance/info, 48/48 route-smoke assertions, full "
                        "suite green, events.jsonl integrity + live-daemon "
                        "healthcheck after every single group) but converting the "
                        "rest is real, repetitive work: each remaining group "
                        "(tracks/dispatch [the crown jewel - touches sessions."
                        "new_track/move_lane/lanemachine], machine, chat/copilot, "
                        "processes, attachments/checkpoints/runs, control/teach, "
                        "connectors, relay/push) needs its own routes_*.py + "
                        "smoke-test coverage before conversion, so a bug in an "
                        "unconverted route stays invisible to test_server_routes.py "
                        "until that group is added.",
        "trigger": "adding a new HTTP route (touches the still-large if-chain); "
                   "any claim the daemon's API surface is fully modular",
        "fix": "server.py: repeat the proven routes_auth.py/.../routes_info.py "
               "pattern for each remaining concern group, one commit each: "
               "(1) add smoke-test coverage for that group's routes to "
               "test_server_routes.py FIRST, (2) move the bodies verbatim into "
               "routes_<concern>.py as (self, user[, body]) functions (watch for "
               "module-level helpers/consts that need to move WITH the group, "
               "e.g. GLASS_BRIEF/_harness_state - both caught by grepping for "
               "remaining callers before deleting the orphan), (3) add the "
               "dispatch-table check (GET_PREFIX_ROUTES for any .startswith() "
               "route), (4) verify full suite + events.jsonl integrity + a "
               "live-daemon curl smoke pass. tracks/dispatch is the largest + "
               "highest-value remaining group but also touches the "
               "sessions.new_track/move_lane/lanemachine crown jewel from the "
               "sessions.py work - do it last, with extra care, once the "
               "smaller groups have built up more route-smoke coverage. "
               "pm.py: the remaining notice functions are lower-ROI extractions "
               "(multiple back-refs for modest clusters) - thread a small State "
               "object (loopstate + _ask/_say/_activity) through them if/when "
               "pm.py's size becomes the active bottleneck again.",
        "order": 31,
    },
    {
        ...
        "fix": "(1) DONE: processes.py migrated into db.py (removes its ROOT/"
               ...
               "(3) INVESTIGATED, NOT a db.py migration for these three - read "
               "all three modules in full (2026-08-18) to check whether the "
               "processes.py pattern (JSON blob -> db table) applies. It does "
               "NOT: connectors.py's CDIR holds the actual EXECUTABLE .py files "
               "(loaded via importlib.util.spec_from_file_location, run in a "
               "separate sandboxed process via subprocess - the file must exist "
               "on disk to import/exec) plus a _versions/ rollback archive of "
               "past file contents; only its small _state.json (last-run "
               "timestamps) is pure JSON and a real migration candidate. "
               "checkpoints.py's CPDIR holds directory-tree SNAPSHOTS (shutil."
               "copytree of settings.json + the whole connectors/ dir, restored "
               "via shutil.copy2) - the value IS the file tree, not a value you'd "
               "unpack into a row. voice.py's CACHE holds binary mp3 files "
               "served by URL (`/glance/voice/<id>.mp3`) and by raw bytes-read "
               "for base64 inlining - forcing these into db.py's id/data-blob-"
               "as-JSON-text table shape would mean base64-encoding audio into "
               "SQLite text columns for no functional gain. Forcing any of the "
               "three into db.py would be a real architecture change (path-vs-"
               "blob semantics differ, not just storage backend) with materially "
               "higher risk than the processes.py port, not a mechanical "
               "extraction - declined for now rather than doing it unsafely. "
               "(4) OPEN, revised plan: add a SHARED test-sandbox helper (a "
               "single `sandbox_daemon_storage(tmp)` function that patches "
               "db.ROOT/db.DBPATH, events.EV/events.SET, auth.USERS/auth.SESS, "
               "AND connectors.CDIR/checkpoints.CPDIR/voice.CACHE all in one "
               "place) so a test author never has to re-derive which globals a "
               "newly-touched module needs sandboxed - migrate connectors.py's "
               "_state.json into db.py separately (small, low-risk, same "
               "pattern) when that module is next touched. (5) OPEN: verify "
               "test integrity checks with md5sum/checksum, not file-size+mtime "
               "- the latter cannot distinguish 'untouched' from 'grew by a "
               "plausible amount'.",
        "order": 32,
    },
]
