...
DEBT = [
    ...
    },
    {
        ...
        "title": "Cells: agentic systems as pluggable units - daemon registry + "
                 "Connectors reference cell shipped (Phase 0+1), Phase 2/3 pending",
        "status": "open",
        ...
        "why_it_bites": "Phase 0+1 cover the registry, daemon gating, and ONE "
                        "conformed cell (Connectors) end-to-end. NOT yet done: "
                        "(1) Copilot/Process/PM still have no kernel Surface "
                        "plugin (profiles enumerate surfaces.pm/processes/... but "
                        "AVAILABLE_PLUGINS has no entry for them yet - same dead-"
                        "reference shape Connectors had before Phase 1). "
                        "(2) per-cell cross-cell-hook guards - a disabled cell's "
                        "event hooks (pm.on_card_done from dispatch/lanemachine, "
                        "processes.clear_step_stamps from move_lane) still fire; "
                        "they need a one-line cells.enabled_id() guard at the ONE "
                        "owning entry point (Phase 2). (3) the Engineer cell's own "
                        "lifecycle (reconciler/watcher) is still a spine call in "
                        "serve(), not registry-driven, and its /tracks routes "
                        "aren't stress-tested under disable (Phase 3, done last - "
                        "it is the gate/merge crown jewel).",
        "trigger": "adding a new agentic system; a UI toggle for a whole system; "
                   "any claim the daemon is fully cell-modular",
        "fix": "Phase 1 DONE (see above - Connectors is now the template to "
               "repeat). Phase 2: Copilot/Process/PM surfaces, same pattern as "
               "connectors.tsx, + the single disabled-guard on each one's "
               "cross-cell hooks. Phase 3: Engineer "
