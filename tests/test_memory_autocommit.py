...
tools/memory_autocommit.py runs after EVERY turn of every interactive session, so
...
Run: py -3.12 tests/test_memory_autocommit.py
"""
import os
import shutil
import subprocess
...
def test_end_to_end_as_the_hook_runs_it():
    ...
    try:
        ...
        runner = (
            ...
            "m.PROJECTS = %r;"
            "sys.exit(m.main())" % (os.path.join(ROOT, "tools"), root)
        )
