...
Named e2e_* so tools/run_gate.py skips it: it binds a port and boots a server.
Run directly:  py -3.12 tests/e2e_lane_http.py
"""
...
sessions._gate = lambda t: (False, [
    "gate command failed (py -3.12 tools/run_gate.py):\nFAIL test_thing.py\nAssertionError: lane != done"])
