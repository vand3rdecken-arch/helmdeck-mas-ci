...
(app/src/ui/card_composer.tsx), which is the same code path the Expo pickers
feed on native.

Prereqs:  py -3.12 tests/uifix_harness.py           (daemon + seeded cards)
          node node_modules/expo/bin/cli export --platform web
    py -3.12 tests/e2e_attach_ui.py <device-token>

Named e2e_* so tools/run_gate.py skips it (needs a port + a browser).
"""
