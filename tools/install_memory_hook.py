...
hook is being installed to solve. So:
    ...
    tools/memory_autocommit.py        canonical - versioned, reviewed, gated
...
memory directory at all (harness/settings/card.json denies Write/Edit there,
measured against the real CLI). Interactive sessions are the only writers, and
they are exactly what this covers.

Usage:
    py -3.12 tools/install_memory_hook.py            # install / update
    py -3.12 tools/install_memory_hook.py --check    # report drift, change nothing
    py -3.12 tools/install_memory_hook.py --uninstall
"""
import json
import os
import shutil
...
def check():
    ...
    if st == "ok":
        ...
        print("         DRIFT - the deployed copy differs from tools/memory_autocommit.py")
