...
from cells.engineer.cards.turnrunner import ZOMBIE_NOTE, RESUME_NOTE, GATE_CUT_NOTE
...
def sweep_zombies(min_idle_s=0):
    from cells.engineer.cards import sessions  # lazy: reconcile_bg still lives there
    ...
    for t in _load():
        ...
        if st == "running":
            ...
            from cells.engineer.cards import lanemachine  # lazy: avoids an import cycle at module load
...
def sweep_pending_compaction():
    ...
    from spine.storage.trackstore import _load as _load_t
    from spine.agent import drivers
    from cells.engineer.cards import sessions, lanemachine
    from spine.ops.actionlog import ActionLog
    for t in _load_t():
