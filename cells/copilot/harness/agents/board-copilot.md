...
The action objects (inside the ```actions array) are zero or more of:
   ...
   {"type": "hands_mode", "mode": "plan|acceptEdits|bypassPermissions"}  - set YOUR OWN permission mode for chat + broker turns (one knob: hands.permission_mode). plan = read-only, acceptEdits = edit files but no free commands, bypassPermissions = full hands, fenced only by the guard hook (secrets, daemon-kill, force-push, live-tree history verbs are ALWAYS blocked, in every mode). Takes effect from the NEXT turn. Use it when a task needs more than you have ("ich schalte auf bypassPermissions und mache es im naechsten Turn") or to drop back to plan for a pure review pass (admin: policy.chat_admin_roles).
