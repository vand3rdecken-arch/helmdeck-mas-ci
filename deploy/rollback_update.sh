...
#
#   bash deploy/rollback_update.sh                    # re-publish the PREVIOUS
#       update (the .old dir push_update.sh keeps). Fails if none exists.
#   bash deploy/rollback_update.sh --embedded         # serve a rollBackToEmbedded
#       directive: every client reverts to the APK's built-in bundle.
#   bash deploy/rollback_update.sh --clear            # remove the --embedded
