...
The content itself lives in `docs/store/ASC_METADATA.md`, not here - this
...
Internal TestFlight testing (this app's fixed scope, docs/ios-requirements.md
...
same as typing them into the App Store Connect web UI and clicking Save.
  ...
  py -3.12 deploy/asc_metadata_draft.py show          # read-only, current ASC state
  py -3.12 deploy/asc_metadata_draft.py apply --yes   # write the draft (PATCH/POST only)

Needs `.env` (ASC_KEY_ID / ASC_ISSUER_ID / ASC_API_KEY_PATH) - same as
deploy/asc_build_state.py and deploy/asc_guide.py. That file is git-ignored
...
# --- the draft content itself (mirrors docs/store/ASC_METADATA.md) --------
...
def cmd_apply(argv):
    if "--yes" not in argv:
        print("refusing to write without --yes (this PATCHes/POSTs live App "
              "Store Connect data - review docs/store/ASC_METADATA.md first)")
        sys.exit(2)
