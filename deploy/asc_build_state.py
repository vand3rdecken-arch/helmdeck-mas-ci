...
Since there is no macOS/Xcode on this box (docs/ios-watch-feasibility.md R2),
...
the artifact - so read it from Apple rather than declaring success at upload.
  ...
  py -3.12 deploy/asc_build_state.py            # one look
  py -3.12 deploy/asc_build_state.py --wait     # poll until Apple stops PROCESSING
