...
# desktop/build-mac.sh (on real macOS) or .github/workflows/desktop-mac.yml
...
# (desktop/native-updater.js), which only ever looks at RELEASE assets - so
...
# wrote from desktop/package.json or its own --version flag).
#
#   bash deploy/release_desktop_mac.sh --dir desktop/release --latest
#   bash deploy/release_desktop_mac.sh --dir /path/to/ci-artifacts --tag v1.0.7
#   bash deploy/release_desktop_mac.sh --dir desktop/release --dry-run
#
...
DIR="desktop/release"; TAG=""; USE_LATEST=0; DRY_RUN=0; NOTES_FILE=""
while [ $# -gt 0 ]; do
  case "$1" in
    --dir)     DIR="${2:-}"; shift ;;
    --tag)     TAG="${2:-}"; shift ;;
...
[ -f "$YML" ] || { echo "!!! no $YML in $DIR - build on macOS first (desktop/build-mac.sh) or point --dir at a downloaded CI artifact set (gh run download <run> -n helmdeck-macos -D DIR)"; exit 1; }
