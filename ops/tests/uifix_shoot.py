...
Prereqs:  py -3.12 ops/tests/uifix_harness.py    (daemon + seeded cards on :8199)
          node node_modules/expo/bin/cli export --platform web   (surfaces/app/dist)

    py -3.12 ops/tests/uifix_shoot.py <device-token>

Serves surfaces/app/dist on :3300, injects the direct-mode config into localStorage
(baseUrl + Bearer token - the same path surfaces/desktop/main.js uses), then shoots the
