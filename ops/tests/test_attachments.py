...
The app sends [{name, data(base64), mime}] (surfaces/app/src/data/attachments.ts); the
...
check(turnopts.MAX_FILES == 6 and turnopts.MAX_BYTES == 5 * 1024 * 1024,
      "caps are MAX_FILES=6 / MAX_BYTES=5MB (surfaces/app/src/data/attachments.ts mirrors these)")
