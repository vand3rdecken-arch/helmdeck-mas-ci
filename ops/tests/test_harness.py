...
Companion to ops/tests/test_harness_layer.py, which holds ops/harness/agents/*.md equal
...
the real ops/harness/ tree is never written to - AGENTS / SETTINGS / VERSIONS are
redirected for every test that writes, and restored in a finally.

Run: py -3.12 ops/tests/test_harness.py
"""
...
class Sandbox:
    ...
    test), so it must not be able to touch the tracked ops/harness/ files - a test
...
def _shipped_schema(sb):
    ...
    for which in ("agent", "settings"):
        shutil.copy(os.path.join(ROOT, "ops", "harness", "schema", "%s.schema.json" % which),
...
def test_write_validates_against_schema():
    print("4. write_agent/write_settings validate against ops/harness/schema/*.json")
