...
ops/deploy/ios_credentials.sh), but Developer ID certs never needed Keychain in
...
scopes are account-wide, not per-platform.
  ...
  py -3.12 ops/deploy/mac_credentials.py --check
  ...
  py -3.12 ops/deploy/mac_credentials.py --create [--out DIR] [--password PW]
  ...
  py -3.12 ops/deploy/mac_credentials.py --finish CERT_PATH [--key KEY_PATH]
                                      [--out DIR] [--password PW]
...
def cmd_create(out_dir, password):
    ...
    try:
        ...
        print("    4. py -3.12 ops/deploy/mac_credentials.py --finish <downloaded>.cer")
        return
