...
# this repo.
#
#   bash ops/deploy/ios_credentials.sh              # FIRST RUN - needs a real terminal
#   bash ops/deploy/ios_credentials.sh --profile internal
#   bash ops/deploy/ios_credentials.sh --check      # preflight only, changes nothing
#   bash ops/deploy/ios_credentials.sh --build      # non-interactive build (agent-safe)
#
# ---------------------------------------------------------------------------
...
#   "C:\Program Files\Git\bin\bash.exe" -lc "cd /c/... && bash ops/deploy/ios_credentials.sh"
#
...
# 6. bash ops/deploy/ios_credentials.sh
set -o pipefail
...
echo "==> EAS account"
( cd surfaces/app && npx --yes eas-cli@latest whoami ) || fail "not logged in - run: npx eas-cli login"
...
if [ "$MODE" = "build" ]; then
  ...
  ( cd surfaces/app && npx --yes eas-cli@latest build --platform ios --profile "$PROFILE" --non-interactive )
  exit $?
fi

# --- first run: create the signing assets ----------------------------------
echo "==> setting up iOS build credentials (no Apple ID prompt in API_KEY mode)"
( cd surfaces/app && npx --yes eas-cli@latest credentials:configure-build --platform ios --profile "$PROFILE" ) || exit 1
...
==> now VERIFY, do not assume:
      ...
      bash ops/deploy/ios_credentials.sh --build
