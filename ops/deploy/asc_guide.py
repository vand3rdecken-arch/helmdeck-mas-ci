...
daemon/browsercap.py, so the Apple session survives between runs.
  ...
  py -3.12 ops/deploy/asc_guide.py open     # launch/attach + go to App Store Connect
  py -3.12 ops/deploy/asc_guide.py shot     # screenshot current tab -> ops/docs/shots/asc.png
  py -3.12 ops/deploy/asc_guide.py where    # url + title + visible headline
  py -3.12 ops/deploy/asc_guide.py apps     # list the apps ASC shows, with their IDs
  py -3.12 ops/deploy/asc_guide.py appid    # print the numeric ascAppId for app.helmdeck
...
def cmd_shot():
    pw, br, ctx = _attach()
    p = _asc_page(ctx)
    out = os.path.join(ROOT, "ops", "docs", "shots", "asc.png")
