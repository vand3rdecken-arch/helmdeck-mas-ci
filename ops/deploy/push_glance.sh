#!/usr/bin/env bash
# Ship the GLASSES webapp: sync surfaces/glasses/*.{html,css,js} into the Glance worker
...
# thing; ops/docs/glasses-reference.md 11.9.) This script produces that URL.
#
...
# ops/deploy/push_update.sh has with the phone.
#
# ONE-TIME SETUP
#   1. Expose the daemon. A Worker runs at Cloudflare's edge and cannot see
#      localhost:8140:
#          bash ops/deploy/cloudflare_tunnel.sh          # prints an https URL
#   2. Tell the Worker where that is (a SECRET - it is a door to the flat):
#          cd surfaces/glasses/worker && npx wrangler secret put DAEMON_URL
...
sync_public() {
  ...
  local f
  for f in "${ASSETS[@]}"; do
    if [ ! -f "$ROOT/surfaces/glasses/$f" ]; then
      echo "FAIL: surfaces/glasses/$f is missing" >&2
      return 1
    fi
    cp "$ROOT/surfaces/glasses/$f" "$PUBLIC/$f"
  done
  echo "  synced ${#ASSETS[@]} files -> surfaces/glasses/worker/public/"
}
...
want_len() { wc -c < "$ROOT/surfaces/glasses/app.js" | tr -d ' '; }
...
verify() {
  ...
  case "$health" in
    ...
    *) echo "WARN: DAEMON_URL is not set on the worker - the app will load but" >&2
       ...
       echo "      cd surfaces/glasses/worker && npx wrangler secret put DAEMON_URL" >&2 ;;
  esac
  want="$(want_len)"; live="$(live_len "$origin")"
  echo "  source app.js: $want bytes"
  echo "  live   app.js: $live bytes"
  if [ "$want" != "$live" ]; then
    echo "FAIL: $origin serves a different build than surfaces/glasses/." >&2
    return 1
  fi
...
echo "== syncing surfaces/glasses/ -> surfaces/glasses/worker/public =="
sync_public
