#!/usr/bin/env bash
# Ship helmdeck.de (the Cloudflare Worker in ops/deploy/waitlist) and PROVE it landed.
#
...
verify() {
  ...
  if [ "$want" != "$live" ]; then
    echo "FAIL: $ORIGIN is serving a different build than ops/deploy/waitlist/src." >&2
    echo "      Run 'bash ops/deploy/push_site.sh' (without --check) to ship it." >&2
    return 1
  fi
...
echo "== deploying ops/deploy/waitlist -> $ORIGIN =="
( cd "$ROOT/ops/deploy/waitlist" && npx wrangler deploy )
