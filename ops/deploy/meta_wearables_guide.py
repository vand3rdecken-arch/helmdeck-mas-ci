...
Same harness standard as ops/deploy/asc_guide.py: attaches over CDP to the
...
(Meta/Facebook login, 2FA, org details, terms acceptance).
  ...
  py -3.12 ops/deploy/meta_wearables_guide.py open <url>   # attach + go to <url>
  py -3.12 ops/deploy/meta_wearables_guide.py shot         # screenshot current tab
  py -3.12 ops/deploy/meta_wearables_guide.py where        # url + title + visible headline
  py -3.12 ops/deploy/meta_wearables_guide.py click <sel>  # click a selector
"""
import os
import subprocess
import sys
...
def cmd_shot():
    ...
    out = os.path.join(ROOT, "ops", "docs", "shots", "meta_wearables.png")
