...
`app/plugins/glassvoice/GlassVoiceService.kt`, `surfaces/app/src/data/voice.ts`,
`ops/docs/voice-interaction-design.md`).
...
`ops/docs/ios-watch-feasibility.md` §2.2) wäre das **kein hartes Veto**, aber ein
Abhängigkeits-Fremdkörper im sonst selbstbestimmten Rollout (Relay-OTA,
`ops/deploy/push_apk.sh`-Analogie), der jederzeit von Google entschieden wird.
...
- **Kein exaktes Zeichenlimit für `ConversationTemplate`-Bubbles gefunden** —
  ...
  `ops/docs/voice-interaction-design.md` §3) liegt in derselben Größenordnung wie
...
- **Kein Hinweis auf Expo-managed vs. hand-verwaltet** — DHU arbeitet auf
  ...
  also identisch gegen HelmDecks hand-verwaltetes `surfaces/app/android` funktionieren,
...
Android-Pfade eingehängt — Prebuild **und** hand-verwaltetes `surfaces/app/android`).
...
(`ops/docs/voice-interaction-design.md` §1: *"Erkennen ist Plattformarbeit,
...
|---|---|---|---|
| 1 | Weg (b): `MessagingStyle`-Notification aus dem bestehenden Push-Pfad (`daemon/notify.py`/`surfaces/app/src/data/push.ts`) ableiten, Reply-`RemoteInput`+Mark-as-read-Action, `IntentService` empfängt die Antwort und ruft denselben `/chat`-Turn wie heute | 1–2 T | größter Teil ist Notification-Umbau, kein neuer Server-Code |
| 2 | DHU-Testaufbau (ADB-Forward gegen ein echtes Telefon, `surfaces/app/android`-Build) | 0,5 T | einmalig |
...
`app/plugins/glassvoice/GlassVoiceService.kt`, `surfaces/app/src/data/voice.ts`,
`ops/docs/voice-interaction-design.md`, `ops/docs/ios-watch-feasibility.md`,
