...
keine Migration. Grundlage: `app/app.json`, `surfaces/app/package.json`, `surfaces/app/src/data/*`
(Transport/Push/OTA/E2EE), `app/plugins/withLanCleartext.js`, `surfaces/relay/relay.py`,
`daemon/notify.py`, `DEPLOY.md`, `ops/deploy/*.sh`, `surfaces/desktop/main.js`,
`surfaces/desktop/electron-builder.yml`, `surfaces/desktop/tray.py`.
...
| `scheme: helmdeck` (Custom-Scheme-Deep-Link) | **läuft unverändert** — deckt QR-/Code-Pairing ab (`surfaces/app/src/app/pair.tsx` nutzt `Linking.useURL()`) |
| `android.intentFilters` für `https://<relay>/pair` (App Links) | **fehlt auf iOS** — Pendant = Universal Links: `ios.associatedDomains` Entitlement **plus** eine `/.well-known/apple-app-site-association`-Datei, die `surfaces/relay/relay.py` ausliefern müsste (kleine Relay-Änderung). Nicht kritisch: QR-Scan und Code-Eingabe funktionieren ohne. |
...
| Modul | Urteil iOS |
|---|---|
| `expo-blur`, `expo-haptics`, `expo-symbols`, `expo-glass-effect`, `@expo/ui` | **laufen unverändert — sind sogar iOS-first** (SF Symbols, Liquid Glass, SwiftUI-Komponenten). Befund am Rande: `expo-glass-effect`, `expo-symbols`, `@expo/ui` sind installiert, aber in `surfaces/app/src` **derzeit ungenutzt** — auf iOS würden sie erstmals Wirkung entfalten. |
...
| `tweetnacl`, `tweetnacl-util`, `qrcode`, `zustand`, `@tanstack/react-query` | **pures JS, läuft unverändert** — die gesamte E2EE (`surfaces/app/src/data/e2ee.ts`, Curve25519/XSalsa20-Poly1305) ist plattformneutral |
| `react-native-sse` | **ungenutzt** — deklariert in package.json, kein Import in `surfaces/app/src` (der SSE-Pfad wurde durch den Long-Poll ersetzt). Wäre ohnehin pures JS. Kandidat zum Entfernen, kein iOS-Thema. |
| `@react-native-async-storage/async-storage` | **läuft unverändert** |

### 1.3 Push — die echte Baustelle Nr. 1

Ist-Zustand (`daemon/notify.py` + `surfaces/app/src/data/push.ts`): der Daemon sendet
...
- **Client:** `expo-updates` + der Silent-OTA-Hook (`surfaces/app/src/data/ota.ts`) sind
...
- **Server:** `surfaces/relay/relay.py` liest bereits `expo-platform`-Header und
...
- **Deploy-Skript:** `ops/deploy/push_update.sh` exportiert hart
...
Verzeichnis (anders als `surfaces/app/android/`) — iOS könnte sauber über
...
| `push_relay.sh` / `/apk/helmdeck.apk` | bekommt **kein** iOS-Pendant — natives Ausliefern läuft über `eas build` + `eas submit` → TestFlight. Neues, kleines Skript (`ops/deploy/push_testflight.sh`) statt Erweiterung. |
...
|---|---|---|---|
| 1 | Apple-Konto, EAS-Setup, `eas.json`, erster TestFlight-Build (managed prebuild neben hand-managed `surfaces/app/android`) | **1–2 T** | einmalig; enthält Credentials-Gefummel |
...
mitzunehmen: `surfaces/desktop/main.js` ist die Electron-Hülle, die den Python-Daemon
...
| lokal servierte SPA (`surfaces/app/dist`, statischer HTTP-Server auf `WEB_PORT`) | reines Node, kein Windows-Bezug. |

### 5.2 Was fehlt

| Lücke | Aufwand |
|---|---|
| `surfaces/desktop/electron-builder.yml` konfiguriert nur `win:` (NSIS) — kein `mac:`-Block (Target `dmg`/`zip`, `category`, `hardenedRuntime: true`, Entitlements-Datei) | klein, ~0,5 T |
...
- der Relay-Transport (`surfaces/app/src/data/client.ts`) + OTA + State/Query.
...
`ops/docs/cards/fix-websearch-permission.md` umgesetzt); ersetzt den früheren
...
[Thunkable (lowcode.agency)](https://www.lowcode.agency/nocode-ops/tools/thunkable),
