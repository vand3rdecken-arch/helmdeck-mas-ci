...
`ops/docs/glasses-reference.md` (§3.1, §4.4, §11.6, §11.7, §11.9).
...
|---|---|---|---|---|
| **Linse** (`surfaces/glasses/`, MRBD-Webview) | ✅ Talk-Antwort + Blocker-Ansage, `mute`/`repeat` (`app.js:378,420`) | ❌ Webview verweigert jede Aufnahme | D-Pad-Auswahl, `/glance/answer` | **produktiv** |
| **Android-Telefon** (Expo RN) | ⛔ keine Audio-Abhängigkeit im Baum (`surfaces/app/package.json`: kein `expo-audio`/`expo-speech`) | ⛔ dito | — | **nicht gebaut** |
...
| **Apple Watch** | ⛔ | ⛔ | — | **nicht analysiert** (`ops/docs/ios-watch-feasibility.md` behandelt nur Push-Mirroring) |
...
1. **`GlassVoiceService.kt` ist ein Torso.** `grep GlassVoice surfaces/app/src` findet
...
2. **Der Mikro-Pfad geht direkt gegen den Daemon** (`URL("$base/glance/talk")`,
   ...
   ungeklärte Frage, die zwischen `surfaces/glasses/` und einer festen Installation
   steht.
...
| Datei | Rolle |
|---|---|
| `surfaces/app/src/data/voice.ts` | Die EINE Naht. `caps()` / `speak()` / `listen()`, je ein Web- und ein Native-Körper. |
| `surfaces/app/src/ui/voice_mode.tsx` | Der Vollbild-Sprachmodus: Orb, Zustandsautomat, Verlauf-Umschalter, drei Regler. |
| `surfaces/app/src/app/chat.tsx` | `ask()` — fährt denselben Turn wie der Composer, nur mit `voice: true`. |
| `surfaces/app/src/ui/card_composer.tsx` | Mikrofonknopf; ersetzt Senden, solange nichts getippt ist (ChatGPT-Muster). |
...
- `npx tsc --noEmit` sauber; `ops/tools/i18n_lint.py` PASS (982 Keys); `run_gate.py` PASS.
- `ops/tools/e2e_voice_playback.py` — ein ECHTER edge-tts-Clip, als `data:`-URI genau
...
- `ops/tools/e2e_voice_loop.py` — **8/8**. Fährt den Zustandsautomaten im echten
...
1. **Android-Paketsichtbarkeit.** Seit API 30 ist ein fremdes Paket unsichtbar,
   ...
   beide Pfade besitzt (Prebuild *und* das handgepflegte `surfaces/app/android`).
2. **Das Vendor-Plugin wurde bewusst NICHT registriert.** Es kann nur Prebuild —
   für `surfaces/app/android` (DEPLOY.md: handgepflegt, regeneriert aus nichts) hat es
...
**Verifiziert:** `ops/tools/e2e_voice_stream.py` **13/13** — füttert den Reply
...
`ops/tools/e2e_voice_loop.py` **12/12** inkl. des Leer-Queue-Falls.
...
ein Electron-Bump hätte dafür nichts gekauft. `ops/tools/probe_duplex.js` hält die
...
`cells/copilot/routes_copilot.py`, `surfaces/glasses/app.js`,
`app/plugins/withGlassVoice.js`, `app/plugins/glassvoice/GlassVoiceService.kt`,
`surfaces/app/package.json`, `ops/docs/glasses-reference.md`, `ops/docs/ios-watch-feasibility.md`.
