...
`ops/docs/paseo-adoption-plan.md`: read the ACTUAL source before building. This
...
- **HelmDeck's `surfaces/glasses/` app is already ~80% conformant** with Meta's real
...
- **Voice is now designed per platform, not only for the lens**:
  `ops/docs/voice-interaction-design.md` (DRAFT, awaiting owner sign-off) carries the
...
project outgrew. When a second surfaces/glasses/companion surface appears, go to tickets;
...
| Default Python UA trips Cloudflare 1010 | `ops/tools/assist_watch.py:14`. |
...
- The **rich627 whatsapp-claude-channel plugin**: send worked, inbound never
  ...
  (`ops/docs/whatsapp-bridge.md:178-180`)
...
`ops/docs/glasses-ux-requirements.md`, nine rules, all learned **on-device
...
### 4.2 The rendering half — `glass-crud-harness/ops/tools/voice_note.py`
...
- If HelmDeck ever accepts replies: only phrases **unambiguous in speech** count
  ...
  usually means 'forget it'."* (`ops/docs/whatsapp-bridge.md:166-168`)
...
3. The bridge is a **patched fork run from source with bun**, needing a second
   WhatsApp number, and `ops/docs/whatsapp-bridge.md:15-17` is blunt about it:
...
4. The bridge daemon must be running for any of this to land. HelmDeck already
   learned the equivalent lesson once (`surfaces/desktop/tray.py` supervisor).
5. **The fork patch file is STALE.** `ops/docs/whatsapp-bridge.fork.patch` is dated
...
From `ops/docs/android-dat-research.md` (deep-research run **2026-07-13/14**, 18
...
glass-crud-harness already uses exactly this (`ops/tools/qr.py:32-35`), including the
...
### 6.5 HelmDeck `surfaces/glasses/` conformance — measured, not assumed
...
| # | Delta | Where |
|---|---|---|
| 1 | **Tap targets below the 88dp minimum**: `.back-btn` 56px, `.field` 48px, `.list-item` ≈66px. Only `.nav-item` (88px) complies. | `surfaces/glasses/styles.css:35,99,61` |
...
| 4 | **No favicon and no `manifest.webmanifest`.** | `surfaces/glasses/index.html` |
...
5. `glass-crud-harness/ops/tools/voice_note.py` — 60 lines, the whole voice recipe.
6. `whatsapp-claude-agent/src/index.ts:245-320` — how the daemon really consumes
   events. **Read the live fork, not `ops/docs/whatsapp-bridge.fork.patch`** (§4.6.5).
...
`ops/deploy/meta_wearables_guide.py`) with the owner completing the actual
...
this doc and the co-pilot script (`ops/deploy/meta_wearables_guide.py`) used to
...
From §2.1 of THIS document — *"when a second surfaces/glasses/companion surface appears,
...
`ops/tools/voice_note.py`: **mp3, not ogg/opus** (that recipe targets a WhatsApp
...
`expo-av`, `expo-audio`, `expo-speech` are all absent from `surfaces/app/package.json`).
...
No dependency was pinned blind: `surfaces/app/node_modules` is EMPTY on this box, so a
...
`ops/tools/qr.py:2-4` is unambiguous that this *adds*, not *previews*: *"Make a QR
...
for the phone (`ops/deploy/push_update.sh`); the glasses deserve the same and
currently do NOT have it.

**What this means for `surfaces/glasses/` — the README is wrong and the gap is real.**
`surfaces/glasses/README.md` said hosting is *"Meta's HTTPS preview"* via
...
research question, and it is the one thing standing between `surfaces/glasses/` and
...
- **Nothing starts it.** A search of `surfaces/app/src` for `GlassVoiceService`,
...
**Verified here (54 assertions, `ops/tests/test_meta_dat_plugin.js`, no network):**
...
`surfaces/app/src` starts either service** — so this is not reachable from the UI and must
...
| **GLASS MODE lens webapp** (`surfaces/glasses/`, blocker list, tappable options) | ❌ **no screen exists** | ✅ |
...
**4. `ops/deploy/build_apk.sh` must apply the plugin.** `surfaces/app/android` is git-ignored
...
the daemon, nothing in `surfaces/app/src` starts either service, and none of this has
