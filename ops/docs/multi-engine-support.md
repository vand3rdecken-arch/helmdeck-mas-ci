...
> **Build plan exists**: `ops/docs/multi-engine-build-plan.md` turns this analysis
> into five dispatchable cards and resolves the §7 open decisions with stated
> defaults. Backlog filing: `ops/docs/backlog/multi-engine-acp/`.

This is the deferred **Phase 5** of `ops/docs/paseo-adoption-plan.md`
...
| Docs | `ops/docs/providers.md`, `ops/docs/custom-providers.md`, `ops/docs/agent-lifecycle.md`, `ops/docs/opencode-global-event-baseline.md` |
...
`TStep` transcript contract (`surfaces/app/src/ui/card_transcript.tsx:24-42`) is already
provider-agnostic. There is even an unused client-side `Engine` plugin interface
waiting for a daemon counterpart (`surfaces/app/src/kernel/keys.ts:17-25`).
...
**HelmDeck's `TStep` (`surfaces/app/src/ui/card_transcript.tsx:24-42`) is already a near-
...
Swept across `daemon/`, `ops/harness/`, `app/`, `ops/tools/`.
...
`probe_harness_settings.py:48`, `ops/tests/probe_cli_askuser.py:25` — all
`CLAUDE = os.environ.get("HELMDECK_CLAUDE") or shutil.which("claude") or …`.
Plus an independent JavaScript reimplementation in `surfaces/desktop/setup.js:110-167`
...
11. Routes `/sessions/claude`, `/sessions/claude/adopt`
    ...
    `adoptClaude` (`surfaces/app/src/data/client.ts:540-541`); `setup.claude`/
    `claudeVersion` (`surfaces/desktop/setup.js:280`, `surfaces/app/src/ui/onboard.tsx:83`).
...
- **Gate.** `lanemachine._gate` (`cells/engineer/lanemachine.py:162-261`)
  ...
  and `helmdeck.gate` exits 0. `ops/tools/run_gate.py` is `py_compile` + an import
...
- **`TStep` UI contract** and the client-side `Engine` plugin interface
  (`surfaces/app/src/kernel/keys.ts:17-25`, `surfaces/app/src/plugins/engines/claude.ts`).
...
| E2 | Launch config | One `resolve_engine_exe(cfg)`; fold the 6 duplicated `CLAUDE` constants + the `surfaces/desktop/setup.js` JS twin; parameterise `_real_claude_exe`; add Paseo's `PARENT_SESSION_ENV_VARS` scrub. | M | 1–1.5 |
...
(`ops/harness/agents/*.md`), the `<helmdeck-ask>` wire protocol, and the
...
- **No engine-string branches in the client.** `surfaces/app/src/types/stream.ts:757-875`
  branches on `event.provider === "claude"` three times — the exact pattern
  Paseo's own `ops/docs/coding-standards.md:69` forbids. HelmDeck's `TStep` contract
...
1. ~~**Which path.**~~ **DECIDED 2026-08-24: full Paseo-equivalent breadth.**
   ...
   `ops/docs/multi-engine-build-plan.md` for the resulting card sequence.
...
- HelmDeck: `spine/agent/{drivers,agentcli,spawnenv,turnopts}.py`,
  ...
  `ops/harness/schema/*.json`, `ops/harness/settings/card.json`,
  `surfaces/app/src/kernel/keys.ts`, `surfaces/app/src/plugins/engines/claude.ts`.
