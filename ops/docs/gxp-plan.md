...
> ungeprüft grün leuchtet. Neue Tests: `ops/tests/test_events_no_reimport.py`,
> `ops/tests/test_auth_audit.py`, `surfaces/app/test_config_hydrate.js`.
>
...
> nur unsere eigene Schranke), und die Datenschutzerklärung in `surfaces/relay/relay.py`
...
Phase B/C/D (GxP-Modus, Signaturen, App) stehen in `ops/docs/gxp-mode-design.md`
...
**Ist.** `surfaces/desktop/main.js`, `mintDesktopToken()`:
...
2. Beim ersten Start zeigt die App den vorhandenen Login-Screen
   (`surfaces/app/src/ui/login_screen.tsx` — existiert, wird nur nicht erreicht).
...
- **Phase B/C/D** — GxP-Modus, Signaturen, App-Oberfläche, Audit-Härtung.
  Beschrieben in `ops/docs/gxp-mode-design.md`. Der offene Punkt dort ist der
