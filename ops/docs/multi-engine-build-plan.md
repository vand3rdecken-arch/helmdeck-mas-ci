...
Executes the recommendation of `ops/docs/multi-engine-support.md` (read that FIRST —
it carries the analysis, the change surface, and the trap register). Same shape
as `ops/docs/paseo-adoption-plan.md`: each phase is a gated worker card
...
- `agentcli.py`: `_real_claude_exe` → parameterised shim resolution
  ...
  `probe_harness_settings.py:48`, `ops/tests/probe_cli_askuser.py:25`).
  `surfaces/desktop/setup.js`'s JS twin stays for Card 5.
...
**Verify**: gate green; full `ops/tests/test_*.py`; `/harness` spawn-preview argv
...
- `claude_sessions.read_transcript_store`/`transcript_store_version` are the
  ...
  COMPLETED blocks).
- Verified with `ops/tools/compare_timeline.py` (new) against real dispatched
...
`surfaces/app/node_modules` isn't set up in this worktree. Recommend the owner spot-
...
- **The E9 probe is this card's second deliverable** — the GO/NO-GO gate:
  ...
  `ops/docs/acp-probe-report.md` with transcripts.
...
- Routes: `/sessions/<engine>` (keep `/sessions/claude` as alias); New-Request
  ...
  (exe resolvable?) and capability badges; `surfaces/desktop/setup.js` probes
...
pass). 18 unit tests (`ops/tests/test_codex_driver.py`) exercise the FULL
...
confidence tier ops/docs/multi-engine-support.md's original analysis was
...
Paseo's source. 25 unit tests (`ops/tests/test_opencode_driver.py`).
...
test (`ops/tests/test_omp_driver.py`, "MULTI-TURN tool loop") reproduces the
...
**Verified**: `ops/tests/test_omp_driver.py` (27 checks, synthetic frames built
...
tests (`ops/tests/test_pi_driver.py`), including one that replays the EXACT
