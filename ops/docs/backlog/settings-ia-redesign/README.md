...
| 2 | `settings-hub-shell` — **ERLEDIGT** (Kernstück) | Hub-Navigator (6 Türen, Drilldown, `?door=` addressierbar), generischer Control-Renderer für Tür 2; alle Panels umgezogen | Kein Knob verlor seine Editierbarkeit; settings.tsx ist jetzt der Hub; automation.tsx = Redirect-Stub; Screenshots Phone+Desktop gejudged (inkl. Redirect-Beweis). **Scope-Korrektur:** kein echter Zweispalter (Paseo-Stil) gebaut — Drilldown bei beiden Breakpoints, maxWidth wie automation.tsx vorher; Tür „Cells" ist ein Link auf das bestehende `modules.tsx` (das bereits ein live registry-getriebener Katalog mit Cell-Diagrammen ist, siehe Karte 3), keine neue Seite; die Sidebar behält vorerst eine redundante „Automation"-Zeile neben „Settings" (Entfernen aus tabs.ts verschoben — Risiko, dass expo-router Tabs unlistete Dateien als Ghost-Tab zeigt, ungeklärt). **Nebenbei gefunden+gefixt:** harness_section.tsx crashte auf einem unvollständigen `/harness`-Dokument (`data.surfaces.find` ohne Array-Guard) — vorbestehender Bug, jetzt defensiv. |
| 3 | `cells-catalog` | Tür 3 ordentlich integrieren statt zu verlinken: Cell-Karten mit Beschreibung/Toggle/Status direkt im Hub; Gesetze-Block sauber getrennt von Cell-Liste; Sidebar-Redundanz (Automation/Modules-Zeilen) aus tabs.ts entfernen, Ghost-Tab-Risiko verifizieren | Jede Cell erklärt sich in einem Satz; modules.tsx als eigener Screen entfällt; Cell-Disable wirkt weiter auf Nav; keine doppelten Sidebar-Einträge mehr |
