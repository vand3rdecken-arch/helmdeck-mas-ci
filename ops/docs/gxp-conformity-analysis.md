...
2. **Das Gate, das das verhindern soll, prüft zwei Dinge: ob Python parst und
   ob der Daemon importiert** (`ops/tools/run_gate.py:45-57`). Bei Änderungen
...
3. **Der Audit-Trail ist Konvention, nicht Mechanik.** Kein Hash-Chain, keine
   Signatur, kein Trigger; `ops/tools/reset.py:64` enthält ein `DELETE FROM events`.
...
mitgeliefert: `ops/tools/reset.py:61-68` enthält `c.execute("DELETE FROM events")`.
...
> **Nebenbefund, der zufällig schützt:** `ops/tools/reset.py:66` bildet den Pfad
...
- **Relay-Verschlüsselung:** echtes Zero-Knowledge. NaCl-Box (Curve25519 +
  ...
  Schlüssel und persistiert nichts (`surfaces/relay/relay.py:5-16`).
- **Cloudflare Worker (Glasses):** Pfad-Allowlist, `Cookie`/`Authorization`
  ...
  manual` (`surfaces/glasses/worker/src/index.js:26-73`). Sauber gebaut.
...
- **`.gitignore`-Abdeckung** für Secrets ist gut, und `ops/deploy/publish_source.sh:170`
...
selbstsigniert (`ops/tools/make_tls_cert.py`), wird von iOS/Android abgelehnt — was
...
`surfaces/relay/relay.py:95` und `:117` sagen zu: *"keine Werbung und keine Analyse-/
Tracking-SDKs"* und *"Nicht verarbeitet werden: … Analysedaten"*. Die App liefert
`posthog-react-native` mit (`surfaces/app/package.json:37`) und ist **Opt-out mit
Default an** (`surfaces/app/src/data/analytics.ts:46`). Zusätzlich überträgt
...
`daemon/` und `surfaces/app/src/` nach `signature`, `re-auth`, `four-eyes`,
...
`surfaces/app/src/ui/board.tsx:594-597` (`NextUp onDone` → `api.moveLane(k.id,"done")`)
...
Verifiziert: `surfaces/desktop/main.js:292-306` ruft bei jedem Start
`python -m spine.auth.mint_token owner desktop` auf und injiziert
`{baseUrl, token}` in den SPA-URL-Fragment (`:367-373`); `surfaces/app/src/data/config.ts:94-96`
...
`helmdeck.gate` ruft `ops/tools/run_gate.py`. Der komplette Prüfumfang:
...
**Konsequenz von (3):** eine Änderung, die nur `app/`, `ops/deploy/`, `ops/docs/`,
`surfaces/relay/`, `web/` oder `surfaces/desktop/` berührt — also praktisch die gesamte
...
`ops/tools/run_suite.py:2-9` sagt es selbst: *„MANUAL tool, nothing schedules or
...
Hooks in `.claude/settings.json` rufen `ops/tools/loop_state.py`, das sich selbst
...
Requirement-ID und keinen Testverweis. `ops/docs/adr/` enthält **einen** ADR.

**GXP-V5 — Kein Artefakt lässt sich auf einen Commit zurückführen. [KRITISCH]**
Kein `git rev-parse HEAD` in `ops/deploy/*.sh`, kein Build-Fingerprint im
...
(`surfaces/app/src/kernel/profiles.ts`) sind UI-Zusammenstellungen, keine Umgebungen.
...
Vorhanden und gut: `ops/deploy/rollback_update.sh` mit drei Stufen und
...
Ausführungsnachweisen, `HARNESS.md`, `ops/docs/voice-interaction-design.md`), aber
...
| GXP-U3 | Desktop-App = Owner-Login ohne Credential | KRITISCH | `surfaces/desktop/main.js:292-306,367-373` |
...
| GXP-A1 | Append-only ist Konvention; `DELETE FROM events` mitgeliefert | KRITISCH | `ops/tools/reset.py:61-68`, `db.py:28-35` |
...
| GXP-V5 | Kein Artefakt auf Commit rückführbar; OTA gar nicht | KRITISCH | `ops/deploy/*.sh`, `DEPLOY.md:57-58` |
...
| GXP-V10 | Keine kontrollierte Dokumentation, keine SOPs | HOCH | `ops/docs/` |
