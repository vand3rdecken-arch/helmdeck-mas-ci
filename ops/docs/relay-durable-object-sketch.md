...
> **Not to be confused with `surfaces/glasses/worker/`, which is real and shipped.** That
...
> different thing: a no-inbound relay that would mirror `surfaces/relay/relay.py`'s
...
The surfaces/glasses/phone work over LAN against the daemon until then; the relay only matters
away-from-home.
