...
Folgedokument zu `ops/docs/gxp-conformity-analysis.md` (Befundregister dort, §8).
...
`ops/tools/reset.py:61-77`, das heute den Audit-Trail löscht: git überlebt das.
...
der ausdrücklichen Regel in `surfaces/app/src/i18n/index.ts:1-11`, dass der Audit-Trail
...
│   → deploy (ops/deploy/push_update.sh)          ← heute unsichtbar│
│                                                               │
│  PRÜFSTAND                                                    │
│   Gate      ✓ bestanden                                       │
...
Neue Zeichenketten gehören nach `surfaces/app/src/i18n/dict/` und brauchen **beide**
...
| Datei | Änderung |
|---|---|
| `surfaces/app/src/ui/sign_off.tsx` | **neu.** Die Maske. Vorbild `card_question.tsx` (Stepper, a11y, Select-then-confirm), Rahmen `prompt_host.tsx:44-77` |
| `surfaces/app/src/data/client.ts` | `signSubject`, `signSession`, `sign` — Muster `answer()` (`:426-430`, Request-ID-Echo). `skipAuthGate=true` bei der Passwortprüfung, sonst wirft ein 401 den Benutzer zum Login (`:104-113`) |
| `surfaces/app/src/ui/board.tsx` | Auslöser `:280-285`, `:414-430`, `:563-577`, `:593-598`; Auswahlmodus |
| `surfaces/app/src/app/card/[id].tsx` | `moveTo` (`:707-725`); Signaturabzeichen |
| `surfaces/app/src/i18n/dict/` | de + en |
...
Damit die Erwartung stimmt. Stand nach `ops/docs/gxp-plan.md`: Stufe 0 ist
...
(`events-two-stores-unreconciled`, bezahlt), `ops/tools/reset.py` verweigert bei
