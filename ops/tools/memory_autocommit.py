...
Canonical copy lives in the HelmDeck repo (ops/tools/) and is deployed to
~/.claude/hooks/ by ops/tools/install_memory_hook.py, which also detects drift
between the two. Fixing untracked state with an untracked script would have been
a joke at its own expense.

Run manually:
    py -3.12 ops/tools/memory_autocommit.py --verbose      # snapshot now
    py -3.12 ops/tools/memory_autocommit.py --dry-run -v   # say what it would do
"""
import json
import os
import subprocess
