...
# (Kotlin) trees. For the current Expo stack use ops/tools/release.sh instead
...
#
#   bash ops/tools/build_all.sh            # build all three
#   bash ops/tools/build_all.sh win        # just the Windows installer
#   bash ops/tools/build_all.sh apk        # just the APK
#   bash ops/tools/build_all.sh glasses    # just zip the glasses webapp
#
...
build_win() {
  ...
  local sevenz="surfaces/desktop/node_modules/7zip-bin/win/x64/7za.exe"
  ...
  ( cd desktop && CSC_IDENTITY_AUTO_DISCOVERY=false node_modules/.bin/electron-builder --win --config electron-builder.yml ) \
    && ok+=("win: $(ls surfaces/desktop/release/*.exe 2>/dev/null | head -1)") || fail+=("win")
}
...
build_glasses() {
  ...
  mkdir -p surfaces/glasses/dist
  # `zip` isn't on Git Bash here; use Python's zipfile (always present).
  py -3.12 -c "import zipfile;z=zipfile.ZipFile('surfaces/glasses/dist/helmdeck-glasses.zip','w',zipfile.ZIP_DEFLATED);[z.write('surfaces/glasses/'+f,f) for f in ('index.html','styles.css','app.js','README.md')];z.close()" \
    && ok+=("glasses: surfaces/glasses/dist/helmdeck-glasses.zip (upload via the toolkit /test-on-device)") || fail+=("glasses")
}
