...
# lints), the self-sandboxed ops/tests/test_*.py (e2e_* skipped - they need
# :3300), and the ops/tests/unit pytest suite. Any failure -> nonzero exit; each
...
if [ -d surfaces/app/node_modules ]; then
  run "app tsc --noEmit" bash -c 'cd surfaces/app && npx tsc --noEmit -p tsconfig.json'
else
  skip+=("app tsc (no surfaces/app/node_modules - run npm install in app/)")
fi

# 3. design lint: the selftest proves the linter, then the linter runs over
#    the dirty files exactly as ops/tools/loop_state.py does in EXECUTE.
if [ -f ops/tools/design_lint_selftest.py ]; then
  run "design_lint selftest" $PY ops/tools/design_lint_selftest.py
fi
...
if [ -d ops/tests/unit ]; then
  if $PY -m pytest --version >/dev/null 2>&1; then
    run "pytest ops/tests/unit" $PY -m pytest ops/tests/unit -q
  else
    skip+=("pytest ops/tests/unit (pytest not installed for $PY)")
  fi
fi
