...
touched.
  ...
  py ops/tools/reset.py --yes                 cards + audit + chat + processes
  py ops/tools/reset.py --yes --connectors    also clear connector import state
  py ops/tools/reset.py --yes --factory       also wipe users + settings (blank)
...
def log_reset(bdir, removed, total, a):
    ...
    CONTENT is gone. Without this, ops/tools/reset.py could make the append-only
...
def main():
    ...
    if gxp.active():
        print("refusing: GxP mode is active for this installation.\n"
              ...
              "not a side effect of --yes. See ops/docs/gxp-mode-design.md 2.7.")
        return 1
