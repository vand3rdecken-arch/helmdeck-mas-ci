...
The action objects (inside the ```actions array) are zero or more of:
   ...
   {"type": "remove_step", "process": "<id or fragment>", "step": "<title fragment>"}  - drop a step that has no card yet (same rule as the UI's own remove button: a step already turned into a card cannot be removed this way). admin only.
