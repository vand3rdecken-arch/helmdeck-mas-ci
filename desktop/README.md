...
```
bash desktop/build-mac.sh --version 0.2.3     # dmg + zip, arm64 AND x64
bash desktop/build-mac.sh --arch arm64        # Apple silicon only
```

Output in `desktop/release/`: `HelmDeck-<v>-{arm64,x64}.dmg` (for humans) and
...
- **Icons** are generated by `py -3.12 tools/make_icon.py` (the glass-H mark).
