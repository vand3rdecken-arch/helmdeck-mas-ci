...
`desktop/tray.py` is the always-on system-tray shell that supervises the daemon
...
pythonw -3.12 desktop/tray.py        # pythonw = no console window
```
...
py -3.12 -m PyInstaller --noconsole --onefile --name HelmDeck desktop/tray.py
# -> dist/HelmDeck.exe
```
