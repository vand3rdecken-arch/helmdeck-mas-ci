...
Deploy: `python surfaces/relay/relay.py` (PORT env, default 6790); front it with TLS.
...
# text is derived from the app's ACTUAL data flows (surfaces/app/src/data/*, this
...
# dir (ops/deploy/push_update.sh); the manifest is rebuilt from disk each request.
#
...
#
# Rollback: a `rollback.json` marker in the served dir (ops/deploy/rollback_update.sh
...
def _bundle_rtv(base_dir):
    ...
    by ops/deploy/push_update.sh as a `runtimeVersion` file). None for a legacy bundle
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p == "/updates/assets":
            ...
            return
        if p.startswith("/apk/"):
            # version.json + the signed APK, uploaded by ops/deploy/push_relay.sh.
