...
// now door 3 of the settings hub instead of a tab of its own
// (accounts-boards-prd phase 4 / settings-ia-redesign phase 3 "cells-catalog").
//
// Lifted verbatim out of (tabs)/modules.tsx, which is a redirect to
// /settings?door=cells now: the content was never the problem (it is already
// a live, registry-driven catalog from GET /cells, with the seeded rules and
// the append-only reconfiguration journal), only its address was. Keeping it
// as a component rather than re-implementing it inside the hub is the point -
// there is still exactly one place a cell's toggle is drawn.
//
// Shows what DeepSeek-style composability means here: the loaded modules
// (engines, surfaces), the SEEDED rules (policies + charter) with live
// toggles, and the journal. Adjusting a policy posts a TRACKED swap to
// /policy/swap.
...
// `t` is the THEME here, so the translator is `tr` - the same aliasing every
// other screen does. Shadowing one with the other is what left this surface
// hardcoded German in the first place.
...
export function CellsCatalog() {
  ...
  // surface still renders the rest of Modules & Rules without crashing.
  const [cells, setCells] = useState<CellInfo[]>([]);
  // Which cell's architecture diagram is showing - SELECT, not
  // expand-in-place, so at most one diagram renders at a time (5 stacked
  // diagrams would blow past the reference style's density target). Tapping
  // the same cell again collapses it.
  ...
  return (
    <View>
      <Text style={{ color: t.txtSecondary, fontSize: 13, marginBottom: 20 }}>{tr("modules.sub")}</Text>

      {err ? <Text style={{ color: t.danger, fontSize: 12, marginBottom: 14 }}>{err}</Text> : null}

      {/* THE CELL CATALOG FIRST (the plan's door 3: "pro Cell eine Karte"),
          the six laws after it. Before this move the order was reversed and
          the cells sat under a block of constitutional toggles - which is
          exactly the "Cells sind unerklärt" complaint the redesign opens
          with. The laws are read-mostly; the cells are what you came for. */}
        ...
        {policies ? (
          <>
            ...
            {/* WIP-Limit dedup (the plan's point D): ONE source is
                ...
                rather than being a second place to type the number. */}
