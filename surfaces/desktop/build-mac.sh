...
#
#   bash surfaces/desktop/build-mac.sh                       # unsigned, both arches
#   bash surfaces/desktop/build-mac.sh --version 0.2.3       # stamp a release version
#   bash surfaces/desktop/build-mac.sh --arch arm64          # Apple silicon only
#   bash surfaces/desktop/build-mac.sh --no-web              # reuse an existing surfaces/app/dist
#
# Produces in surfaces/desktop/release/:
...
# the same App Store Connect API key ops/deploy/ios_credentials.sh already uses,
...
if [ -n "${CSC_LINK:-}${CSC_NAME:-}" ]; then
  ...
  if [ -n "${ASC_API_KEY_PATH:-}" ] && [ -n "${ASC_KEY_ID:-}" ] \
    ...
    # convention (ops/deploy/ios_credentials.sh, .env) is ASC_* - map one to the other
...
echo ""
echo "DONE. Artifacts -> surfaces/desktop/release/"
