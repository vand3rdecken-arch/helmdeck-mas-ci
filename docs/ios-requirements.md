...
`docs/ios-watch-feasibility.md` (Commit `ed1c25d`) und **gegen den aktuellen
Baum verifiziert** (`app/app.json` v1.0.3, `app/package.json`,
`app/src/data/push.ts`, `app/src/data/attachments.ts`, `app/eas.json`).
...
| `android.intentFilters` (`https://…/pair`) | iOS-Pendant = Universal Links (`ios.associatedDomains` + AASA-Datei auf dem Relay, ~20 Zeilen `relay/relay.py`). **Optional / verschoben** — QR-Scan + Code-Eingabe decken Pairing ab. |
| `react-native-sse` | ungenutzt (kein Import in `app/src`, verifiziert) — Aufräum-Kandidat, **kein** iOS-Blocker. |
...
- [ ] `app/src/data/push.ts`: auf iOS `getExpoPushTokenAsync({ projectId })`
...
- [ ] Neues kleines Skript `deploy/push_testflight.sh` (`eas build` +
