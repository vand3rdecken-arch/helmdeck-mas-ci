...
>
> **Stale where noted:** `web/` (Next.js) and `apk/` (Kotlin) were superseded
> by the single Expo app in `app/` - phone, web and desktop from one codebase -
> and now live under `archive/`. The `daemon/`, `glasses/` and `worker/` rows
> still describe the current tree. `ARCHITECTURE.md` is the authority, and
