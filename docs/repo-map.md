...
|---|---|---|
| `spine/` | Shared daemon infrastructure no cell owns: auth+charter+policy+gxp, http server+routes, storage, registry (cells/debt/harness/escalations), agent drivers, turn, media, comms, git/worktrees, ops. | Every daemon request, live process on `:8140`. |
...
| `desktop/` | Electron shell, tray supervisor source, electron-builder config, entitlements. | Desktop release builds + the packaged app at `C:\HelmDeck` runs copies. |
| `glasses/` | Ray-Ban Display webapp (3 real files) + its Cloudflare worker; `worker/public/` is GENERATED by `deploy/push_glance.sh`. | The shipped glance worker. |
| `relay/` | `relay.py` - the stateless transport deployed to the Oracle VM. | Phone<->daemon traffic; deployed via `push_relay.sh`. |
| `deploy/` | One entrypoint per surface (`build_apk`, `release_desktop*`, `push_update`, `push_glance`, `push_relay`, `push_site`, `publish_source`) + credential guides. | Operator + fast-track deploy hook. |
| `tools/` | `loop_state.py` (runs in the Claude-Code hooks of every session), `run_gate.py` (the daemon runs it at every gate), `reset.py`, linters, icon maker. | Live hooks + the gate - runtime, not just dev. |
| `tests/` | Replayable pins (manual `run_suite.py`, never per-card by decree) + `smoke/` e2e harness. | Operators/agents on demand. |
| `harness/` | Policy-as-data: agent briefs (`agents/*.md`) + settings the daemon resolves at EVERY spawn. `.versions/` (local) is undo history. | Daemon runtime, owner-editable. |
| `docs/` | Internal knowledge (public-mirror-FILTERED by owner decree) + `docs/backlog/` = the live card queue (tracked via gitignore negation) + `docs/shots/`. | Owner workflow, card dispatch, mandatory reading rules. |
...
py -3.12 tools/run_gate.py            # quick check: compile + wiring
```
