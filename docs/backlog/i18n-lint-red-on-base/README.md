...
(its diff touches zero `app/src` files); found while gating it and split off on
...
`tools/run_gate.py` runs `tools/i18n_lint.py` as a gate check, and it currently
...
## The punch list (`py -3.12 tools/i18n_lint.py`)

Rule B - German prose literal in `app/src` outside the dicts (14):

```
app/src/app/(tabs)/modules.tsx:97   'Agent-Backends hinter einem Kontrakt (Claude / Copilot / Dee…'
app/src/app/(tabs)/modules.tsx:102  'verfügbar'
app/src/app/(tabs)/modules.tsx:102  'aus'
app/src/app/(tabs)/modules.tsx:107  '${surfaces.length} registrierte Oberflächen (Nav aus der Reg…'
app/src/app/(tabs)/modules.tsx:111  'Standard = heutiger Charter. Umschalten schreibt einen getra…'
app/src/app/(tabs)/modules.tsx:114  'Suite muss grün sein, bevor reviewt wird'
app/src/app/(tabs)/modules.tsx:118  'Gemessene Ökonomie'
app/src/app/(tabs)/modules.tsx:119  'Aus = Human-Bestätigung nötig'
app/src/app/(tabs)/modules.tsx:120  'laufende Karten'
app/src/app/(tabs)/modules.tsx:125  'Agentische Systeme (Rolle + Route + UI-Surface + Enable-Flag…'
app/src/app/(tabs)/modules.tsx:147  'Quelle: ${charter.source} — selbst ein tauschbares Modul.'
app/src/app/(tabs)/modules.tsx:154  'Jede Modul-/Regeländerung, mit Urheber (die einzige Invarian…'
app/src/app/(tabs)/modules.tsx:156  'von ${e.actor}${e.replaced ? …'
app/src/ui/cell_diagram.tsx:39      'keine Routen'
```

Rule D - JSX text node with real prose, unwrapped (6):

```
app/src/app/(tabs)/modules.tsx:90   'Module & Regeln'
app/src/app/(tabs)/modules.tsx:104  'Kein Kernel — Fallback aktiv.'
app/src/app/(tabs)/modules.tsx:135  'Keine Cells geladen.'
app/src/app/(tabs)/modules.tsx:157  'Noch keine Einträge.'
app/src/app/kernel-demo.tsx:20      'no renderable surface'
app/src/ui/login_screen.tsx:84      'Konto erstellen'
```
...
Add keys to a dict in `app/src/i18n/dict/` (`screens.ts` is the right home for
...
shadow it - alias the translator the way `app/src/ui/board.tsx:15` does:
...
```
py -3.12 tools/i18n_lint.py                 # must print "i18n lint: N keys" and exit 0
cd app && npx tsc --noEmit                  # dict keys are typed
```
