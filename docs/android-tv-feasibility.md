...
dieses Repos eigene Dateien (`app/src/app/pair.tsx`, `app/src/app/scan.tsx`,
`app/src/data/qrgen.ts`, `app/src/data/qrgen.web.ts`, `app/src/data/push.ts`,
`app/plugins/withGlassVoice.js`, `app/app.json`, `app/package.json`).

**Kurzfazit vorweg:** Anders als bei Android Auto (`docs/android-auto-feasibility.md`)
...
- **Keine Kategorie-Sperre gefunden** — die Seite schließt Chat-/
  ...
  `docs/android-auto-feasibility.md` §3).
...
- Pflicht-Banner: **320×180 px, xhdpi-Ordner, Text im Bild eingebrannt, pro
  ...
  (`app/src/i18n`) — also zwei Banner-Varianten nötig, zusätzlich zum
...
- **`expo-router` läuft mittlerweile auf TV** (früher, laut dem 2024er
  ...
  genau HelmDecks Stack (Expo Router, file-based). Da `app/src/app` bereits
...
- **Für HelmDeck heißt das:** Der proaktive "Henry spricht eine Push-
  ...
  `speak`, referenziert in `docs/glasses-reference.md` §11.8) hat auf einem
...
Der Kernunterschied zu `docs/android-auto-feasibility.md` §5: dort verbietet
...
Aus diesem Repo gelesen: `app/src/app/pair.tsx`, `app/src/app/scan.tsx`,
`app/src/data/qrgen.ts`, `app/src/data/qrgen.web.ts`, `app/src/ui/onboard.tsx`,
`app/src/data/push.ts`, `app/app.json`, `app/package.json`, `app/src/i18n`.
...
Kartenlauf zu diesem Thema, wie schon `docs/android-auto-feasibility.md` für
Android Auto.
