...
| Baustein | Wo |
|---|---|
| Datenschutz-URL (live nach Relay-Deploy) | `https://141.144.227.105.sslip.io/privacy` — vorher einmal `bash deploy/push_relay.sh` |
| Data-Safety-Antworten | `docs/store/DATA_SAFETY.md` (Tabelle §2 abtippen) |
| Kurz-/Langbeschreibung DE+EN | `docs/store/LISTING.md` |
| Screenshots (1080×2400) | `docs/store/screenshots/01…04*.png` |
| Technik-Checkliste (Signing, Cleartext, assetlinks) | `app/PLAY_STORE_RELEASE.md` |
...
2. `deploy/push_relay.sh` einmal ausführen → `/privacy` ist öffentlich.
...
2. **Data safety**: Tabelle aus `docs/store/DATA_SAFETY.md` §2 übernehmen
...
1. **Main store listing**: Texte aus `docs/store/LISTING.md` (DE primär,
   EN als Übersetzung anlegen), Screenshots aus `docs/store/screenshots/`
...
2. Fingerprint in `relay/relay.py` `ASSETLINKS` ergänzen (der bestehende
   Eintrag ist der Sideload-Key; der Play-Key kommt DAZU) →
   `bash deploy/push_relay.sh` → `adb shell pm get-app-links app.helmdeck`
...
3. `eas submit -p android` für Folge-Uploads: Service-Account-Key anlegen
   (siehe `app/PLAY_STORE_RELEASE.md` §8) und in `eas.json` referenzieren.

## 7. Smoke auf dem Internal-Build (vor Promotion)

- [ ] Pairing per QR (Relay-Modus) und direkt (HTTPS) — Achtung: Play-Build
  blockt Cleartext-HTTP, LAN-Empfehlung siehe `app/PLAY_STORE_RELEASE.md` §7
