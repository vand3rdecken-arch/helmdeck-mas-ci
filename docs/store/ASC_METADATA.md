...
Datei ist die Zuarbeit für `deploy/asc_metadata_draft.py apply` bzw. für
...
Fixierter Scope (`docs/ios-requirements.md` §1/§7): **TestFlight, internes
...
zurückgestellt, bis ein Store-Release ansteht — dann `docs/store/LISTING.md`
...
Live-Werte (siehe `DEPLOY.md` §2b/§2c, `deploy/asc_build_state.py`):
...
XSalsa20-Poly1305, `app/src/data/e2ee.ts`) — das ist mehr als reines
...
| `contactEmail` | tienduyvo@googlemail.com | Kontakt-E-Mail aus `docs/store/LISTING.md` |
...
Store-Listing. Basis: `docs/store/LISTING.md` Kurzbeschreibung, gekürzt/
...
⚠ Die Privacy-Policy-Seite (`relay/relay.py PRIVACY_HTML`) ist noch auf dem
Android/FCM-Stand von `docs/store/DATA_SAFETY.md` geschrieben — sie erwähnt
weder PostHog-Analytics (seit `[NEU]` in `docs/ios-requirements.md` §2) noch
Apple/APNs (Push läuft auf iOS aktuell noch über den rohen FCM-Token,
Umbau auf Expo Push ist offener Punkt in `docs/ios-requirements.md` §3).
...
- Kein volles Store-Listing (Kategorie, Keywords, Screenshots) — außerhalb
  des fixierten Scopes (`docs/ios-requirements.md` §7).

## 6. Anwenden

```bash
py -3.12 deploy/asc_metadata_draft.py show     # liest aktuellen ASC-Stand, ändert nichts
py -3.12 deploy/asc_metadata_draft.py apply --yes   # schreibt die Entwürfe oben (PATCH/POST, kein Review-Submit)
```
