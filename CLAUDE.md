...
Touching the GLASSES layer - `surfaces/glasses/`, `/glance`, voice output, proactive
notification, a companion app, the Meta SDK? **`ops/docs/glasses-reference.md` is
...
same rule as `ops/docs/paseo-adoption-plan.md`.

## The build loop (enforced by hooks)

This repo has the same loop control as the glass harness. State is computed
from disk by `python ops/tools/loop_state.py`; a Stop hook blocks resting while an
...
- JS/React/asset change → OTA: `bash ops/deploy/push_update.sh` (seconds).
- Native change (native module, permission, app.json plugin, runtimeVersion) →
  ...
  `ops/deploy/push_relay.sh` AND a matching OTA (else the old relay bundle reverts
